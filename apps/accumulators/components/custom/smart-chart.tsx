 ...
 * that — only `ws`/`symbol`/`barriers`/callbacks, all tick-stable (`barriers` is
 * memoised on the proposal). `memo` skips tick re-renders so the Flutter chart
 * isn't churned several times a second.
 */
function SmartChartWrapperImpl({
  ...
  // "Retrieving Market Symbols…" forever. Fix: wait for the socket to actually be
  // OPEN before sending, so the very first request succeeds.
  ...
  // surface inits wrong and the chart never paints. Toggling devtools "fixes" it
  // because it changes the actual element dimensions. So the nudge must cause a
  // REAL dimension change: momentarily add 1px to the Flutter host height and
  // restore it next frame, repeating until a canvas actually paints.
