      ...
      {open && riseFall && streaks && (
        <div className="mt-2 flex flex-col gap-2">
          ...
          <div className="flex gap-3 text-[11px] text-muted-foreground">
            <span>avg move <span className="font-semibold text-foreground">{riseFall.avgMove.toFixed(5)}</span></span>
            <span className="ml-auto">
              {currentDir ? (
                <span className={currentDir.type === 'rise' ? 'text-emerald-500' : 'text-red-500'}>
                  {currentDir.type === 'rise' ? '↑' : '↓'} {currentDir.length} in a row
                </span>
              ) : (
                <span>↑{streaks.longestRise} ↓{streaks.longestFall} longest</span>
              )}
            </span>
          </div>
